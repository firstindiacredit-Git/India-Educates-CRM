...
import CustomColorPicker, {
  isLightColor,
} from "./colorpicker/CustomColorPicker";
...
const ProjectDashboard = () => {
    ...
    const fetchTotalProjects = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}api/totalProjects`
        );
  ...
  useEffect(() => {
    const fetchTotalClients = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}api/totalClients`
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchTotalEmployees = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}api/totalEmployees`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  useEffect(() => {
    const fetchProjectStatusCounts = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}api/projectStatusCounts`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}api/adminNotePad/${email}`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}api/adminTodoList/${email}`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  useEffect(() => {
    const fetchExcelSheet = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}api/adminExcelSheet/${email}`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  useEffect(() => {
    const fetchColors = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_BASE_URL}api/adminColors/${email}`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  const projectsChartData = createChartData(
    "Projects",
    totalProjects,
    "rgba(255, 99, 132, 0.6)"
  );
  const clientsChartData = createChartData(
    "Associates",
    totalClients,
    "rgba(54, 162, 235, 0.6)"
  );
  const employeesChartData = createChartData(
    "Agents",
    totalEmployees,
    "rgba(75, 192, 192, 0.6)"
  );
  ...
  const deleteColumn = async (tableIndex, colIndex) => {
    try {
      ...
      updatedTables[tableIndex].data = updatedTables[tableIndex].data.map(
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      switch (deleteAction.type) {
        ...
        case "table":
          const newTables = [...tables];
          newTables[deleteAction.index].data = Array(
            newTables[deleteAction.index].rows
          )
            .fill()
          ...
          await fetch(
            ...
            }api/adminExcelSheet/${excelSheetId}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ tables: newTables, email }),
  ...
  const handleConfirm = async (confirmation) => {
    ...
    if (confirmation && selectedHoliday) {
      ...
      toast.success(
        `Tomorrow is a holiday of ${selectedHoliday.name} on ${selectedHoliday.date.iso}. Enjoy your day off!`
      );
    } else {
      await axios.post(`${import.meta.env.VITE_BASE_URL}api/notifyHoliday`, {
        isConfirmed: confirmation,
      ...
      toast.info(
        "There is no holiday tomorrow. Please come on time, all employees."
      );
    }
  };
  ...
  const handleApplyFormula = async (tableIndex, rowIndex, colIndex) => {
    try {
      ...
      const formulaValue =
        typeof currentCell === "object" ? currentCell.formula : currentCell;
        ...
        if (startRow + rowIndex < tables[tableIndex].rows) {
          row.forEach((cell, colIndex) => {
            if (startCol + colIndex < tables[tableIndex].cols) {
              newTables[tableIndex].data[startRow + rowIndex][
                startCol + colIndex
              ] = cell;
            }
          });
  ...
  const handleApplyFormulaToSelection = async (tableIndex) => {
    ...
    try {
      const newTables = [...tables];
      const startCell =
        newTables[tableIndex].data[selectedCells.start.rowIndex][
          selectedCells.start.colIndex
        ];

      // Check if the first cell contains a DATESERIES formula
      if (
        typeof startCell === "string" &&
        startCell.startsWith("=DATESERIES")
      ) {
        const params = startCell
          .match(/DATESERIES\((.*)\)/i)[1]
          .split(",")
          .map((x) => x.trim());
        const startDate = new Date(params[0]);
        const count =
          Math.abs(
            selectedCells.end.rowIndex - selectedCells.start.rowIndex + 1
          ) *
          Math.abs(
            selectedCells.end.colIndex - selectedCells.start.colIndex + 1
          );
        const increment = parseInt(params[2]) || 1;
                          ...
                          >
                            <div
                              ...
                              style={{
                                ...
                                left: "5px",
                                top: "10px",
                                color: isLightColor(notepadColor)
                                  ? "#666"
                                  : "#ccc",
                            ...
                            <textarea
                              ...
                              style={{
                                ...
                                backgroundImage: `linear-gradient(${
                                  isLightColor(notepadColor)
                                    ? "#adb5bd"
                                    : "#ffffff33"
                                } 1px, transparent 1px), linear-gradient(90deg, transparent 0px, transparent 1px, transparent 1px)`,
                        ...
                        <div className="d-flex justify-content-between align-items-center mt-3">
                          ...
                          <div className="position-relative btn-group">
                            <button
                              ...
                              onClick={() =>
                                setShowNotePadPicker(!showNotePadPicker)
                              }
                            ...
                            <div className="btn-group">
                              <button
                                className={`btn btn-sm ${
                                  isBold ? "btn-secondary" : "btn-outline-dark"
                                }`}
                                onClick={toggleBold}
                                title="Toggle Bold"
                              >
                          ...
                          <div className="btn-group">
                            ...
                            <button
                              className={`btn btn-sm me-1 ${
                                isSpeaking ? "btn-danger" : "btn-secondary"
                              }`}
                              onClick={speakText}
                            >
                              <i
                          ...
                          <>
                            {tables.map((table, tableIndex) => (
                              <div
                                ...
                                style={{
                                  backgroundColor:
                                    excelSheetColors[table.id] || "#d4edda",
                                  borderRadius: "8px",
                                  padding: "15px",
                                }}
                              >
                                ...
                                <div
                                  ...
                                  style={{
                                    maxHeight:
                                      table.rows > 10 ? "400px" : "auto",
                                    overflowY:
                                      table.rows > 10 ? "auto" : "visible",
                                    overflowX: "auto",
                                    // msOverflowStyle: 'none',
                                    // scrollbarWidth: 'none',
                                    // '&::-webkit-scrollbar': {
                                ...
                                <div className="mb-4 d-flex justify-content-between">
                                  <div className="position-relative btn-group">
                                    <button
                                      ...
                                      onClick={() =>
                                        setShowTableColorPicker(table.id)
                                      }
                        ...
                        {decisionMade ? (
                          ...
                          <div>
                            <h4
                              ...
                              style={{ color: isDarkMode ? "#fff" : "#000" }}
                            >
                              Do you want to declare tomorrow as an office
                              holiday?
                            </h4>
                            <div className="">
                              <button
                      ...
                      {selectedHoliday ? (
                        <div className="card" style={darkModeStyles.card}>
                          <div
                            ...
                            style={darkModeStyles.cardHeader}
                          >
                          ...
                          <div className="card-body">
                            ...
                            >
                              <thead
                                ...
                                style={darkModeStyles.tableHeader}
                              >
                                <tr>
                                  <th>Name</th>
                                  <th>Date</th>
